"""Generate MoonProto proposal PDF — one-page compact style."""
...
class PDF(FPDF):
    ...
    def title_line(self):
        ...
        self.cell(0, 7, "MoonProto 项目申报书", align="C", new_x="LMARGIN", new_y="NEXT")
        self.set_font("F", "", 8)
...
pdf.sec("一", "基本信息")
pdf.info("项目名称",
         "MoonProto：Protocol Buffers 运行时编码解码库（Varint/Zigzag/Wire Types/Encoder/Decoder/Packed/Proto Parser）")
pdf.info("GitHub", "https://github.com/Duan525/moonproto")
pdf.info("GitLink", "待创建")
pdf.info("项目方向", "MoonBit 基础库 / 序列化协议基础设施")
pdf.info("移植参考", "Go protowire、Rust prost::encoding 设计思路｜许可证：MIT")

# ===== 二 =====
pdf.sec("二", "项目简介")
pdf.body(
    "MoonProto 是一个纯 MoonBit 实现的 Protocol Buffers 运行时编码解码库，支持完整的 protobuf wire format。"
...
for item in [
    "Varint: ULEB128 编解码（0~2^64-1），Zigzag 32/64 有符号映射，sint32/sint64 组合编解码",
    "Wire: tag 编解码（make_tag/split_tag），4 种 wire type 读写（Varint/Fixed64/Length-delimited/Fixed32），边界检查",
    "Encoder: 20 个方法（int32/64, uint32/64, sint32/64, fixed32/64, sfixed32/64, float, double, bool, string, bytes, message），Builder 链式调用",
    "Decoder: 19 个方法（read_tag 驱动循环，按字段号分发，skip_field 跳过未知字段，read_message 嵌套解码）",
...
pdf.body("MoonProto vs 生态现状：")
pdf.t_header(["维度", "MoonBit 现有", "MoonProto"], [46, 50, 50])
for row in [
    ("Protobuf 实现", "不存在", "完整 wire format 实现"),
    ("Varint/Zigzag", "仅 LEB128（buffer 包）", "完整 protobuf varint + 解码"),
...
pdf.body(
    ...
    "MoonProto 的实现使 MoonBit 项目能够与 Go/Rust/Java/Python 等语言的 protobuf 实现进行跨语言数据交换。"
)
...
pdf.body(
    "总计 2,279 行，80 测试全通过，15 次有效提交，CI 已配置。"
...
output_path = os.path.join(os.path.dirname(__file__), "MoonProto项目申报书.pdf")
